...
xdim = 640
ydim = 480
#print("video",video,vide)
if video == "None":
    stream = cv2.VideoCapture('v4l2src device=/dev/video'+camid+' ! video/x-raw,framerate=20/1,width='+str(xdim)+',height='+str(ydim)+' ! videoscale ! videoconvert ! appsink', cv2.CAP_GSTREAMER)
else:
    stream = cv2.VideoCapture("videos/"+video)
    #stream.set(cv2.CAP_PROP_FRAME_WIDTH, xdim)
    #stream.set(cv2.CAP_PROP_FRAME_HEIGHT, ydim)
    #stream = cv2.VideoCapture('filesrc location=videos/'+video+' ! decodebin name=dec ! videoscale ! videoconvert ! appsink', cv2.CAP_GSTREAMER)
    #stream = cv2.VideoCapture('playbin uri=file:///home/pi/frc-exercise-helper/videos/'+video+' ! appsink', cv2.CAP_GSTREAMER)
...
humanReadable = {"other":"Please wait","situp":"Good situp", "situp-bad":"Keep your legs still", "situp-bad2":"Do not tug on your neck", "situp-bad3":"Go up further", "pushup":"Good pushup", "pushup-bad":"Keep your back horizontal", "pushup-bad2":"Keep your knees above the ground", "squat":"Good squat","squat-bad":"Go down further"}
...
def runcam():
    ...
    #cv2.setWindowProperty('Exercise Helper', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    cv2.namedWindow("Exercise Helper")
    cv2.moveWindow("Exercise Helper", 0, 0)
    t0 = time.time()
    while True:
        _, frame = stream.read()
        ...
        try:
            ...
            cv2.putText(frame,str(humanReadable[revLookup[int(eid)]]), (10, 20), font, 1, (0, 0, 0), 2)
            cv2.putText(frame,"X", (620, 20), font, 1, (0, 0, 0), 2)

            if time.time()-t0 > 7:
                os.system("espeak '"+str(humanReadable[revLookup[int(eid)]])+"' &")
                t0 = time.time()
            ...
            cv2.setMouseCallback("Exercise Helper", on_mouse_click, frame)
